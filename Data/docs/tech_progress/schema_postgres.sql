-- PostgreSQL 15+ schema (optimized)
...
create index if not exists idx_task_category on task(task_category_id);
create index if not exists idx_link_week on task_tech_link(week);
create index if not exists idx_link_task on task_tech_link(task_id);
create index if not exists idx_link_tech on task_tech_link(tech_id);
...
create index if not exists idx_scope_active_task_week on scope_active_task(week);
create index if not exists idx_llm_task_card_week on llm_task_card(week);

-- Prevent more than one active scope per week
create unique index if not exists ux_scope_active_week_active
  on scope_active(week)
  where status = 'active';

-- Prevent duplicate links for same evidence
create unique index if not exists ux_link_dedupe
